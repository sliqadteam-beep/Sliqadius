...
const CONFIG_URL='google-config.js?v=11';
...
function storeToken(t,expiresIn){safeSet(K_TOKEN,t,true);safeSet(K_TOKEN_EXP,String(Date.now()+Math.max(60,Number(expiresIn||3600)-30)*1000),true)}
...
async function load(){
  ...
  loadPromise=new Promise((resolve,reject)=>{
    ...
    const s=document.createElement('script');s.src='https://accounts.google.com/gsi/client';s.async=true;s.defer=true;s.dataset.sliqGoogle='1';s.onload=resolve;s.onerror=()=>reject(new Error('GOOGLE_LOAD_FAILED'));document.head.appendChild(s)
  });
  return loadPromise
}
...
function signOut(){const token=getToken();if(token&&global.google&&global.google.accounts&&global.google.accounts.oauth2){try{global.google.accounts.oauth2.revoke(token,()=>{})}catch(e){}}safeRemove(K_TOKEN,true);safeRemove(K_TOKEN_EXP,true);saveProfile(null)}

async function driveFetch(url,opts){const token=getToken();if(!token){const e=new Error('GOOGLE_TOKEN_REQUIRED');e.code='GOOGLE_TOKEN_REQUIRED';throw e}opts=opts||{};opts.headers=Object.assign({},opts.headers||{},{Authorization:'Bearer '+token});opts.cache='no-store';const r=await fetch(url,opts);if(r.status===401){safeRemove(K_TOKEN,true);safeRemove(K_TOKEN_EXP,true)}return r}
...
function setupMessage(code){const de=langCode()==='de';if(code.indexOf('HTTPS_REQUIRED')>=0)return de?'Google-Anmeldung braucht HTTPS. Aktiviere zuerst „Enforce HTTPS“ für sliqado.org.':'Google sign-in requires HTTPS first.';if(code.indexOf('NO_CLIENT_ID')>=0)return de?'Google-Anmeldung ist fast fertig. Die Google Web-Client-ID muss noch einmalig für sliqado.org eingetragen werden.':'Google sign-in is almost ready. The Google Web Client ID still needs to be configured once.';return de?'Google-Anmeldung konnte nicht geöffnet werden. Bitte Pop-ups erlauben und erneut versuchen.':'Google sign-in could not open. Allow pop-ups and try again.'}
...
global.SliqGoogle={getClientId,setClientId,getProfile,getToken,loadConfig,load,signIn,signOut,readAppData,writeAppData,isReady:function(){return!!getClientId()},hasSession:function(){return!!getToken()}};
})(window);
