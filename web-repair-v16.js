...
if(window.__SLIQ_WEB_REPAIR_V16__)return;
window.__SLIQ_WEB_REPAIR_V16__=true;
...
function addVoiceToPlusMenu(){
  ...
  b.onclick=function(e){
    ...
    var input=$('messageInput');if(input)input.focus();
  };
  menu.appendChild(b);
}
...
function installAutoScrollSafety(){
  var box=$('messages');
  if(!box||box.dataset.sliqRepairScroll==='1')return;
  box.dataset.sliqRepairScroll='1';
  var nearBottom=true;
  box.addEventListener('scroll',function(){nearBottom=(box.scrollHeight-box.scrollTop-box.clientHeight)<180},{passive:true});
  var timer=0;
...
function keepKeyPromptCorrect(){
  var modal=$('keyModal');
  if(!modal)return;
  if(getKey()&&modal.classList.contains('open')){
    var status=$('keyStatus');
    if(status&&status.textContent===KEY_REQUIRED[lang()])status.textContent='';
  }
}
...
function syncLanguage(){
  var b=$('sliqVoiceMenuBtn');if(b)b.textContent=VOICE[lang()]||VOICE.en;
}
